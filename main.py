...
def load_vgg(sess, vgg_path):
    ...
    #print(" input layer shape: {} ".format(vgg_input.get_shape()));

    #tf.Print(vgg_input, [tf.shape(vgg_input)])
    ...
    return vgg_input, vgg_keep_prob, vgg_l3, vgg_l4, vgg_l7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    #Kernel initializer with normal distribution. use in all layers.
    initializer = tf.truncated_normal_initializer(stddev=0.01) 
    
    #l7 1x1 conv layer
    dec_l7_1x1 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1,
                                  padding = 'same',
                                  kernel_regularizer =
    ...
    dec_l7_tx = tf.layers.conv2d_transpose(dec_l7_1x1, num_classes, 4, 2,
                                           padding = 'same',
                                           kernel_regularizer =
                                           tf.contrib.layers.l2_regularizer(1e-3),
                                           kernel_initializer = initializer)
    
    dec_l4_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1,
                                  padding = 'same',
                                  kernel_regularizer =
                                  tf.contrib.layers.l2_regularizer(1e-3),
                                  kernel_initializer = initializer)
    ...
    dec_l4l7 = tf.add(dec_l7_tx, dec_l4_1x1)

    dec_l4l7_tx = tf.layers.conv2d_transpose(dec_l4l7, num_classes, 4, 2,
                                             padding = 'same',
                                             kernel_regularizer =
                                             tf.contrib.layers.l2_regularizer(1e-3),
                                             kernel_initializer = initializer)

    dec_l3_1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1,
                                  padding = 'same',
                                  kernel_regularizer =
                                  tf.contrib.layers.l2_regularizer(1e-3),
                                  kernel_initializer = initializer)
    
    dec_l3l4l7 = tf.add(dec_l4l7_tx, dec_l3_1x1)

    dec_l3l4l7_tx = tf.layers.conv2d_transpose(dec_l3l4l7, num_classes, 16, 8,
                                               padding = 'same',
                                               kernel_regularizer =
                                               tf.contrib.layers.l2_regularizer(1e-3),
                                               kernel_initializer = initializer)
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    #Flattend from 4D to 2D. TBD: Understand better
    logits = tf.reshape(nn_last_layer, (-1, num_classes))

    # TBD - Reshape correct_label?
...
def train_nn(sess, epochs, batch_size, get_batches_fn,
             ...
             input_image,
             correct_label,
             keep_prob, #keep_prob and learning_rate - unused
             learning_rate):
    """
...
def run():
    num_classes  = 2
    image_shape  = (160, 576) #TBD - 92,160 flattened 1 channel?
    image_channels = 3
    data_dir = './data'
    runs_dir = './runs'
    ...
    with tf.Session() as sess:
        ...
        vgg_input, keep_prob, vgg_l3, vgg_l4, vgg_l7 = load_vgg(sess,
                                                                vgg_path)
        ...
        train_nn(sess, epochs, batch_size, get_batches_fn,
                 training_operation, loss_operation, vgg_input,
